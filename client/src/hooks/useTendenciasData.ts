...
export interface LocationData {
  name: string;
  weeklyValues: number[]; // index 0 = week 1
  ytd: number;
}
...
interface SheetConfig {
  ...
  // Monthly columns vary per sheet - these are the column indices (1-based)
...
const SHEET_CONFIGS: Record<SupportType, SheetConfig> = {
  Remotos: {
    ...
    monthly2026Col: 2, // col B
    monthly2025Col: 24, // col X
...
// Weekly data columns: C (col 3) to BB (col 54) = weeks 1-52
const WEEK_COL_START = 4; // col D (1-based), week 1
const WEEK_COL_END = 55;  // col BC (1-based), week 52
// Note: In some sheets, week 1 starts at col D (index 4), in others at col B or C.
// Based on verified data: weeks are in cols D onwards (col 4 in 1-based)
// Except Remotos which uses col B (2) onwards for weekly data
...
function parseSheet(workbook: XLSX.WorkBook, type: SupportType): SheetData {
  ...
  // Detect column range for weekly data by reading the header row
  // Use the row ABOVE the 2026 data row to find week numbers
  const headerRow = config.weekly2026Row - 1;
  let weekColStart = 0;
  let weekColEnd = 0;

  // Find first column with a week number (1)
  for (let c = 2; c <= 55; c++) {
    const val = getCellValue(sheet, headerRow, c);
    if (val === 1) { weekColStart = c; break; }
  }
  // Find last week column (up to 52, but stop at col 55/BB)
  ...
  if (config.locationsRowStart !== null && config.locationsRowEnd !== null) {
    for (let r = config.locationsRowStart; r <= config.locationsRowEnd; r++) {
      const name = getCellString(sheet, r, 3); // col C = location name
      if (!name) continue;
  ...
  for (let r = config.categoriesRowStart; r <= config.categoriesRowEnd; r++) {
    const group = getCellString(sheet, r, 2); // col B
    const resolution = getCellString(sheet, r, 3); // col C
...
function parseTendenciasWorkbook(workbook: XLSX.WorkBook): TendenciasData {
...
export function useTendenciasData() {
  ...
  const loadFromFile = useCallback(async (file: File): Promise<LoadTendenciasResult> => {
    ...
    try {
      ...
      const result = parseTendenciasWorkbook(workbook);
      setData(result);
      return { success: true };
    } catch (err) {
